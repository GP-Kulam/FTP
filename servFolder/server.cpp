...
void write_file(string file_name, int sock_fd){
    ...
    write_res_file << data;
    cout << "File succesfully stored" << endl;
}
...
void recieveFile(int client_sock_fd){
    char res_file[20];
    recv(client_sock_fd, res_file, sizeof(res_file), 0);
    int i = 0;
    cout << "File you are uploading is: ";
    while(res_file[i] != '\0'){
        cout << res_file[i];
        i++;
    }
    cout << endl;
    write_file(res_file, client_sock_fd);
}
int main(){
    ...
    switch(req[0]){
        ...
        break;
        case '2': recieveFile(client_sock_fd);
        break;
